...
| `assert_semantic(actual, criteria, judge)` | `(string, string, string)` | uses the AI backend as a judge; tape-recorded |
...
| `http` | `get(url)`, `post(url, body)`, `put(url, body)`, `patch(url, body)`, `delete(url)`, `req`, `resp`, `header`, `query`, `body<T>` |
...
| Receiver | Methods |
|---|---|
| `list<T>` | `.len()`, `.empty()`, `.first()`, `.last()`, `.slice(a, b)`, `.contains(x)`, `.join(sep)`. On `var` bindings: `.push(x) -> int`, `.pop() -> option<T>` |
