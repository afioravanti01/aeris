...
| M47 | v0.3 — `aeris install` — per-user project store | New CLI verb `aeris install <dir>`: copy a project directory into `$HOME/.aeris/projects/<name>`, where `<name>` is the `[project] name` from the directory's `aeris.toml`. The copy is recursive; re-installing the same name removes-then-replaces (never merges); `name` is validated as a single safe path component so a crafted manifest can never escape the store; a self-install (source == destination) is refused before the destructive remove. Reuses `manifest::parse_manifest`; manifest problems exit `69`, filesystem failures exit `1`. Documented in `language.md § 25.6` and `cheatsheet.md § 15.1` | 1 | M7 | done |
...
| M22.T12 | `docker` lifecycle surface — extends the M22.T6 family so a `saga` can drive a real deploy with typed compensations instead of shelling out. New ops `docker.logs(name)` (read), `docker.stop(name)`, `docker.rm(target)`, `docker.rmi(image)` (write), plus optional trailing args: `docker.run(image, name)` runs detached + named (`run -d --name`) where 1-arg stays `run --rm`, and `docker.build(context, tag)` tags the build (`build -t`). Touches `DockerBackend` trait + `Mock`/`Real`/`Loaded` impls, `eval.rs` dispatch + builtins + mock/real bodies + kwargs param-names, and `check/effects.rs` (`ALL_CAPS` + `WRITE_OPS`). Partially lands the deferred M22.T1 (`docker.logs` and friends). Surface is documented in language.md § 23 + cheatsheet § 13/§ 16 and exercised by `demo/07-docker-deploy` | `cargo test`: 1004 lib green; `demo/07-docker-deploy` runs against a live daemon (build → rm → run → inspect gate → compensating stop/rmi) with `docker_*` trace events and no `shell_exec` | § 23 / § 12 | done |
...
### 5.31 M47 — `aeris install` — per-user project store (1 week)

`aeris init` scaffolds a project in place and `aeris run` executes one
from any path, but there was no way to publish a finished project to a
stable, well-known location. M47 adds `aeris install <dir>`: it reads
the directory's `aeris.toml`, takes `[project] name`, and copies the
whole tree to `$HOME/.aeris/projects/<name>` — the per-user store other
tooling can rely on.

| Task | Description | Acceptance | Spec ref | Status |
|---|---|---|---|---|
| M47.T1 | CLI surface: add the `Install { path }` subcommand to the clap `Command` enum and dispatch it to `cmd_install` in `run()`. `cmd_install` resolves `$HOME` (error if unset/empty), builds `$HOME/.aeris/projects`, delegates to the testable `install_project(source, projects_root)`, and maps the result to exit codes: `0` on success (prints `aeris: installed <dest>`), `69` for any manifest problem, `1` for filesystem errors | `aeris install --help` lists the verb; `aeris install <dir>` on a valid project prints the install path and exits 0; the directory appears under `$HOME/.aeris/projects/<name>` | § 25.1 / § 25.6 | done |
| M47.T2 | Core copy + name resolution: `install_project` verifies `source` is a directory, reads `<source>/aeris.toml`, parses it via `manifest::parse_manifest`, derives `name` from `[project]`, and recursively copies `source` into `<projects_root>/<name>`. Re-install removes the existing destination first (replace, never merge). A self-install (canonicalised source == destination) is refused before the remove so the source is never wiped | Unit tests: copies under the manifest name; a re-install drops a stale file from the previous copy; missing `aeris.toml` → `Manifest` error | § 25.6 | done |
| M47.T3 | Name safety: `validate_project_name` rejects empty, `.`, `..`, any value containing `/`, `\`, or NUL, and anything that does not resolve to exactly one `Component::Normal`, so a crafted `name = "../escape"` cannot write outside the store | Unit tests: `demo`, `my-aeris-project`, `app_2` accepted; `""`, `.`, `..`, `a/b`, `../x`, `/abs` rejected; an end-to-end install of a `../escape` manifest leaves no directory outside the store | § 25.6 | done |
| M47.T4 | Docs: add `aeris install <dir>` to `language.md § 25.1`, write the new `§ 25.6` (store location, replace semantics, name-safety rule, exit codes), and add the `aeris install` row to `cheatsheet.md § 15.1`. Record M47 in `plan.md § 3` and this task table | `language.md` / `cheatsheet.md` cross-referenced and in sync; `cargo test --lib install` and `validate_project_name` green | § 25.6 | done |
